...
const spouseBlock = (over = {}) => ({
  enabled: true, name: "",
  incomes: [], holdings: { mf: 0, fd: 0, rd: 0, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 },
  ...over,
});

const spousePlan = (over = {}) => plan({
  currentAge: 30, lifeExpectancy: 40, retirementAge: 60,
  incomes: [income({ amount: 100000 })],
  ...over,
});

/* The whole point of a default-off flag: adding the block must not move a
   single number until the user asks for it. */
test("a recorded but uncombined spouse changes nothing at all", () => {
  const without = spousePlan();
  const withSpouse = spousePlan({
    spouse: spouseBlock({
      incomes: [income({ id: 9, amount: 80000 })],
      holdings: { mf: 500000, fd: 0, rd: 0, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 },
    }),
  });
  assert.deepEqual(runProjectionV1(withSpouse), runProjectionV1(without));
});

test("combining adds exactly the spouse's income to the row", () => {
  const base = spousePlan();
  const combined = spousePlan({
    combineSpouse: true,
    spouse: spouseBlock({ incomes: [income({ id: 9, amount: 80000, growthRate: 0 })] }),
  });
  const a = runProjectionV1(base).data[1].income;
  const b = runProjectionV1(combined).data[1].income;
  assert.equal(b - a, 80000 * 12);
});

test("combining adds the spouse's holdings to the opening portfolio", () => {
  const base = spousePlan();
  const combined = spousePlan({
    combineSpouse: true,
    spouse: spouseBlock({
      holdings: { mf: 300000, fd: 100000, rd: 50000, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 },
    }),
  });
  const delta = runProjectionV1(combined).openingPortfolio -
                runProjectionV1(base).openingPortfolio;
  assert.equal(delta, 450000);
  /* FD and RD must land in the single fdrd bucket, not be counted twice. */
  const b = runProjectionV1(combined).data[0].buckets;
  const a = runProjectionV1(base).data[0].buckets;
  assert.equal(b.fdrd - a.fdrd, 150000);
  assert.equal(b.mf - a.mf, 300000);
});

/* Both flags gate the merge, so switching the block off is as effective as
   unticking the toggle — and cannot strand combined-but-hidden money. */
test("a disabled spouse is ignored even when combineSpouse is on", () => {
  const off = spousePlan({
    combineSpouse: true,
    spouse: spouseBlock({
      enabled: false,
      incomes: [income({ id: 9, amount: 80000 })],
  ...
  assert.deepEqual(runProjectionV1(off), runProjectionV1(spousePlan()));
});

test("a spouse's own retireAge stops their income, not the primary's", () => {
  const p = spousePlan({
    combineSpouse: true,
    incomes: [income({ amount: 100000, retireAge: 60 })],
    spouse: spouseBlock({ incomes: [income({ id: 9, amount: 80000, retireAge: 35 })] }),
  });
  const rows = runProjectionV1(p).data;
