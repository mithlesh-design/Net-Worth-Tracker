...
/* Whether the spouse's money is part of this projection. Both flags matter: a
   user can record a spouse and choose not to combine, or combine and later
   switch the whole block off. Exported so the UI shows the toggle on exactly
   the same condition the engine acts on. */
export const spouseIsCombined = (plan) =>
  !!plan?.combineSpouse && !!plan?.spouse?.enabled;
...
export function openingBuckets(plan) {
  ...
  /* A combined spouse's holdings are added per bucket. Life insurance stays
     primary-only: there is no spouse.lifeInsurance to read. */
  const s = spouseIsCombined(plan) ? (plan.spouse?.holdings ?? {}) : {};
  ...
  return {
    mf: num(h.mf) + num(s.mf),
    stocks: num(h.stocks) + num(s.stocks),
    crypto: num(h.crypto) + num(s.crypto),
    // the intake sheet's single FD/RD figure
    fdrd: num(h.fd) + num(h.rd) + num(s.fd) + num(s.rd),
    ppf: num(h.ppf) + num(s.ppf),
    epf: num(h.epf) + num(s.epf),
    nps: num(h.nps) + num(s.nps),
    lic: licCash,
    unallocated: num(h.unallocated) + num(s.unallocated),
...
export function blendedReturn(plan, B) {
  const invested = Object.keys(B).filter((b) => BUCKET_DEFS[b]?.tier !== ILLIQUID);
  const total = invested.reduce((s, b) => s + B[b], 0);
  if (total <= EPS) return num(plan.expectedXIRR);
  return invested.reduce((s, b) => s + B[b] * bucketReturn(plan, b), 0) / total;
}
...
export function runProjectionV1(plan, opts) {
  ...
  const propertyState = buildPropertyState(plan);

  const incomeSources = spouseIsCombined(plan)
    ? [...(plan.incomes ?? []), ...(plan.spouse?.incomes ?? [])]
    : (plan.incomes ?? []);
  ...
  for (let y = 0; y <= years; y++) {
    ...
    /* 2. INCOME — take-home only, so nothing is taxed here. Every amount is
          ...
          BEHAVIOUR-NOTES. */
    let income = 0;
    /* Each source carries its own retireAge, so a spouse working to 60 while
       the primary stops at 55 keeps earning — but `working` still follows the
       single plan.retirementAge, so contributions stop and the post-retirement
       return cap engages at 55 for both. That is the existing one-retirement
       model, not a spouse-specific rule. */
    for (const inc of incomeSources) {
      if (age >= num(inc.retireAge)) continue;
      income += toAnnual(num(inc.amount), inc.frequency) *
    ...
    /* Owned property is charged against the SAME lines as goal property, so the
       ...
       got there. Only net worth distinguishes them. */
    const owned = propertyYear(propertyState, y, B.property);
    const totalEMI = goalLoans.totalEMI + owned.emi;
    const maintenanceCost = goalLoans.maintenanceCost + owned.maintenance;
    const totalPropertyValue = goalLoans.totalPropertyValue + B.property;
    const totalLoanOutstanding = goalLoans.totalLoanOutstanding + owned.debt;
    ...
    const rentalIncome = owned.rent * Math.pow(1 + num(plan.inflationRate) / 100, y);
    ...
    const illiquidNW = tierSum(B, ILLIQUID);
    ...
    const totalNW = liquidNW + lockedNW + illiquidNW - owned.debt - cumUnfunded;
    peakNW = Math.max(peakNW, totalNW);
    ...
    data.push({
      ...
      propertyDebt: Math.round(owned.debt),
