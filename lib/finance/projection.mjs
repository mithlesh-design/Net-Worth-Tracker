...
export function buildGoalLoanState(plan) {
  return (plan.goals ?? []).map((g) => {
    if (!g.hasLoan) return null;
    const dp = num(g.amount) * (num(g.downPaymentPct) / 100);
    const loanAmt = num(g.amount) - dp;
    if (loanAmt <= 0) return null;
    return {
...
/* Cash due for goals landing on this exact age: the down payment when financed,
   the full cost otherwise. NET — drawFromBuckets applies the gross-up. */
export const goalsDueAt = (plan, age) => (plan.goals ?? [])
  .filter((g) => num(g.age) === age)
  .reduce((s, g) => s + (g.hasLoan ? num(g.amount) * (num(g.downPaymentPct) / 100) : num(g.amount)), 0);
...
export function runProjectionV1(plan) {
  ...
  const goalLoanState = buildGoalLoanState(plan);
  ...
  for (let y = 0; y <= years; y++) {
    ...
    const goalNet = goalsDueAt(plan, age);
