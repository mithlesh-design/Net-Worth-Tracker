...
export function computeSuccessScore(plan, opts = {}) {
  ...
  /* Each bucket is drawn around its OWNER's assumptions: in a household the
     engine hands returnFor that person's plan. For a single-person plan the
     owner is `plan` itself, so these are the same numbers as always. Cached
     per owner object — the engine passes the same one every year of a run. */
  const assumptions = new Map();
  const assumptionsOf = (owner) => {
    let a = assumptions.get(owner);
    if (!a) {
      a = { mu: {}, sd: {}, postRetire: num(owner.postRetireReturn) };
      for (const b of Object.keys(BUCKET_DEFS)) {
        a.mu[b] = bucketReturn(owner, b);
        a.sd[b] = bucketVolatility(owner, b);
      }
      assumptions.set(owner, a);
    }
    return a;
  };
  ...
  for (let run = 0; run < runs; run++) {
    /* One shock per YEAR, shared across every bucket — and across every
       person in a household, since they live through the same market —
       scaled by each bucket's own sigma.
    ...
    const out = runProjectionV1(plan, {
      ...
      returnFor: (b, y, ctx) => {
        const { mu, sd, postRetire } = assumptionsOf(ctx.plan ?? plan);
        let m = mu[b], v = sd[b];
