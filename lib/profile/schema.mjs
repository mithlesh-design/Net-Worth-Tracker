...
export const DEFAULTS = {
  ...
  /* ── Client fields 3-5 ── */
  /* Amounts are take-home: what actually reaches the bank. There is no tax
     model, so a figure entered here is never reduced again. */
  incomes: [
    { id: 1, name: "Salary", role: "salary", amount: 137433, frequency: "monthly",
      growthRate: 10, retireAge: 55 },
    { id: 2, name: "Other Income", role: "other", amount: 0, frequency: "monthly",
      growthRate: 0, retireAge: 55 },
  ],

  /* A spouse or partner as a parallel earner, not a merged household.

     Kept as its own block rather than an `owner` field on each income so that
     excluding them from the projection can never lose the data, and so their
     retirement age stays legible next to their own income rather than hidden in
     a filtered list.

     No `contributions` here, deliberately. buildContributionPlan keys named
     streams by instrument, so a second contributions object would collide on
     `mfSip`; worse, it resolves aggregate-vs-detailed globally, so a spouse with
     any detailed amount would flip the primary off their aggregate SIP slider
     and silently drop it from the projection. A spouse SIP goes in
     contributions.other[] until that is reworked per-person.

     Sub-fields added to spouse.incomes[] later will NOT backfill into saved
     profiles: fillDefaults takes arrays wholesale. Read array-item fields
     through num(x, default) at use time, as buildGoalLoanState does. */
  spouse: {
    enabled: false,
    name: "",
    /* Same shape as incomes[] above, and take-home for the same reason. */
    incomes: [],
  ...
  /* Whether the spouse's income and holdings enter the projection. Default
     false for the same reason as inflateGoals: recording someone is not the
     same as deciding to plan jointly, and a migration should not make that
     choice. Read by the engine rather than filtered in page.jsx, because
     useSuccessScore takes the raw plan and clamps it independently — a filter
     applied at the page would be bypassed there and the Strategy Hub score
     would silently disagree with the Review chart.

     Household expenses remain a single figure, so combining two incomes
     without combining expenses overstates surplus by the spouse's share of
     living costs. There is no honest engine fix short of per-person expenses;
     the UI says so where the toggle is. */
  combineSpouse: false,

  /* Property you own TODAY. A home you plan to BUY is a goal, not an entry
     here — the goal machinery already charges its down payment, EMI and
     maintenance. The two describe disjoint periods (owned from today vs bought
     at a future age), which is what makes double-counting structurally
     impossible rather than merely discouraged. See BEHAVIOUR-NOTES.

     Each item:
       { id, name, value, rentalIncome, maintenancePct,
         loanOutstanding, loanRate, loanRemainingYears }

     There is no per-property appreciation rate. Property appreciates at the
     `property` bucket rate like every other asset class, so it also takes a
     volatility draw in the Success Score; a per-item rate applied directly
     would bypass that and model a house as risk-free.

     loanOutstanding is the balance left TODAY, not the original principal, and
     loanRemainingYears the years left to run — for a property already owned,
     those are the numbers the user actually has to hand.

     Every field is read through num(x, default) at use time: fillDefaults takes
     arrays wholesale, so sub-fields added later would never backfill into a
     saved item. */
  properties: [],
