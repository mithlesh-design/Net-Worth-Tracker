...
/* ── The spouse block becomes a household member (v2 -> v3) ── */

/* A v2 document as the v2 app saved it, spouse block and all. */
const v2 = (spouse, over = {}) => {
  const p = { ...migrate(v0), schemaVersion: 2, ...over };
  delete p.members;
  delete p.household;
  if (spouse !== undefined) p.spouse = spouse;
  return p;
};
const spouse = (over = {}) => ({
  enabled: true, name: "Priya",
  incomes: [{ id: 11, name: "Salary", role: "salary", amount: 70000, frequency: "monthly", growthRate: 7, retireAge: 58 }],
  holdings: { mf: 800000, fd: 0, rd: 0, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 },
  ...over,
});

test("a combined spouse becomes one included member", () => {
  const out = migrate(v2(spouse(), { combineSpouse: true }));
  assert.equal(out.members.length, 1);
  const m = out.members[0];
  assert.equal(m.relationship, "spouse");
  assert.equal(m.included, true);
  assert.equal(m.personal.fullName, "Priya");
  assert.equal(m.incomes[0].amount, 70000);
  assert.equal(m.holdings.mf, 800000);
});

test("an enabled but uncombined spouse arrives excluded", () => {
  const m = migrate(v2(spouse(), { combineSpouse: false })).members[0];
  assert.equal(m.included, false);
});

/* Nothing is lost: the data survives even though v2 was ignoring it. */
test("a disabled spouse holding data arrives excluded, data intact", () => {
  const out = migrate(v2(spouse({ enabled: false }), { combineSpouse: true }));
  assert.equal(out.members.length, 1);
  assert.equal(out.members[0].included, false);
  assert.equal(out.members[0].holdings.mf, 800000);
});

test("an empty, disabled spouse block is just the old default and becomes nothing", () => {
  const empty = spouse({ enabled: false, name: "", incomes: [],
    holdings: { mf: 0, fd: 0, rd: 0, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 } });
  assert.deepEqual(migrate(v2(empty)).members, []);
  assert.deepEqual(migrate(v2(undefined)).members, []);
});

/* What keeps the household projection still: v2 ran their money on your
   assumptions, so the member starts with a copy of them. */
test("a migrated spouse takes your assumptions, your retirement age and, provisionally, your age", () => {
  const src = v2(spouse(), {
    combineSpouse: true, expectedXIRR: 10.5, exitTaxRate: 15, retirementAge: 52,
    ppfOpenedYear: 2016, bucketOverrides: { crypto: { exitTax: 30 } },
  });
  const m = migrate(src).members[0];
  assert.equal(m.expectedXIRR, 10.5);
  assert.equal(m.exitTaxRate, 15);
  assert.equal(m.retirementAge, 52);
  assert.equal(m.ppfOpenedYear, 2016);
  assert.deepEqual(m.bucketOverrides, { crypto: { exitTax: 30 } });
  assert.equal(m.currentAge, null);
  assert.equal(m.ageAssumed, true);
});

test("the spouse block and its flag are gone after migration", () => {
  const out = migrate(v2(spouse(), { combineSpouse: true }));
  assert.ok(!("spouse" in out));
  assert.ok(!("combineSpouse" in out));
  assert.deepEqual(out.household, { includeSelf: true, selfCostShare: 100 });
});

/* Stored rows are never rewritten, so a v2 profile is migrated on every load
   and must come out the same every time — including after a birthday, which
   is why the step reads no date. */
test("the v2 -> v3 step is idempotent", () => {
  const once = migrate(v2(spouse(), { combineSpouse: true, personal: { fullName: "", dob: "1990-03-01" } }));
  const twice = migrate(serialize(once));
