...
const spouse = (over = {}) => ({
  enabled: true, name: "", incomes: [],
  holdings: { mf: 0, fd: 0, rd: 0, ppf: 0, nps: 0, epf: 0, stocks: 0, crypto: 0, unallocated: 0 },
  ...over,
});

test("a spouse income is validated on its own path", () => {
  const p = plan({ spouse: spouse({
    incomes: [{ id: 1, name: "Her salary", amount: -5, frequency: "monthly", growthRate: 0, retireAge: 60 }],
  }) });
  assert.equal(at(validate(p), "spouse.incomes.0.amount").length, 1);
});

test("a spouse holding cannot be negative", () => {
  const p = plan({ spouse: spouse({ holdings: { mf: -1 } }) });
  assert.equal(at(validate(p), "spouse.holdings.mf").length, 1);
});

/* The dead end this guards: saveProfile refuses on hasErrors, so an error on a
   card the user has switched off and can no longer see would leave them unable
   to save with no way to find the cause. */
test("a disabled spouse raises no findings, however bad the data", () => {
  const p = plan({ spouse: spouse({
    enabled: false,
    incomes: [{ id: 1, name: "X", amount: -5, frequency: "monthly", growthRate: 999, retireAge: 60 }],
    holdings: { mf: -1 },
  }) });
  const spouseFindings = validate(p).filter((f) => f.path.startsWith("spouse."));
  assert.deepEqual(spouseFindings, []);
  assert.equal(hasErrors(validate(p)), false);
});

test("spouse holdings are clamped even while the block is disabled", () => {
  const p = plan({ spouse: spouse({ enabled: false, holdings: { mf: -100, fd: Number.NaN } }) });
  const c = clampPlan(p);
  assert.equal(c.spouse.holdings.mf, 0);
  assert.equal(c.spouse.holdings.fd, 0);
});

test("clampPlan survives a profile with no spouse block at all", () => {
  const p = plan();
  delete p.spouse;
