...
export function validate(plan) {
  ...
  /* ── Income ──
     Factored out so the primary and spouse lists cannot drift apart. */
  const checkIncome = (inc, p) => {
  ...
  asArray(plan.incomes).forEach((inc, i) => checkIncome(inc, `incomes.${i}`));

  /* ── Spouse ──
     Gated on enabled. Without the gate, someone who fills in spouse details and
     then turns the block off is left with errors on a card they can no longer
     see, and saveProfile refuses on hasErrors — an unrecoverable dead end. */
  if (plan.spouse?.enabled) {
    asArray(plan.spouse.incomes).forEach((inc, i) => checkIncome(inc, `spouse.incomes.${i}`));
    for (const [k, v] of Object.entries(asObject(plan.spouse.holdings))) {
      if (!isNum(v) || v < 0) add(`spouse.holdings.${k}`, "Current value cannot be negative.");
    }
  }
...
export function clampPlan(plan) {
  ...
  /* Clamped whether or not the spouse is enabled: a disabled block contributes
     nothing to the projection, but a NaN left sitting in it would propagate the
     moment it is switched on. */
  for (const k of Object.keys(asObject(p.spouse?.holdings))) {
    p.spouse.holdings[k] = Math.max(0, isNum(p.spouse.holdings[k]) ? p.spouse.holdings[k] : 0);
  }
  return p;
}
